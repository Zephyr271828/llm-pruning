...
def get_partitions_orig(num_samples: int,
    ...
    # print(f'samples_per_canonical_node: {samples_per_canonical_node}')
    # print(f'canonical: {num_canonical_nodes}, physical: {num_physical_nodes}')
    if num_canonical_nodes < num_physical_nodes:
    ...
    # padded_samples_per_canonical_node = max(samples_per_canonical_node + padding, ranks_per_node)
    ...
    if used_ids is not None:
        ...
        tot_ranks = num_physical_nodes * ranks_per_node
        empty_num = tot_ranks - len(ids) % tot_ranks
        ids = np.concatenate([ids, np.full(empty_num, -1, np.int64)])
    ###########################################
    # NOTE temporarily disabled due to shape mismatch
    # print("ids shape", ids.shape)
    # print("drop first", drop_first)
    ...
    if overflow:
        ...
        # print('id shape:', ids.shape)
    # print('PHYSICAL NODES:', num_physical_nodes)
    # print('RANKS PER NODE:', ranks_per_node)
